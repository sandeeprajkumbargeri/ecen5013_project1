...
void *light_sense_task_thread(void *args)
{
  ...
  char log_message[128] = {0};
    ...
    sprintf(log_message, "## LIGHT ## Error opening the I2C Character Device. %s", strerror(errno));
  ...
  {
    bzero(log_message, sizeof(log_message));
    sprintf(log_message, "## LIGHT ## Error accesing I2C bus. %s", strerror(errno));
    LOG(mq_logger, log_message);
  ...
  {
    bzero(log_message, sizeof(log_message));
    sprintf(log_message, "## LIGHT ## Error initializing the APDS 9301 sensor. %s", strerror(errno));
    LOG(mq_logger, log_message);
  ...
  {
    bzero(log_message, sizeof(log_message));
    sprintf(log_message, "## LIGHT ## Unable to send heartbeat.");
    LOG(mq_logger, log_message);
  }
  ...
  while(1)
	{
      ...
      retval = i2c_bus_access(i2c_bus_desc, SLAVE_ADDRESS);
			...
			{
        bzero(log_message, sizeof(log_message));
        sprintf(log_message, "## LIGHT ## Error reading ADC Channel 0 Register.");
        LOG(mq_logger, log_message);
      ...
      adc1 = apds_9301_read_adcn(i2c_bus_desc, ADC_CHANNEL_1);
			...
			{
        bzero(log_message, sizeof(log_message));
        sprintf(log_message, "## LIGHT ## Error reading ADC Channel 1 Register.");
        LOG(mq_logger, log_message);
      ...
      bzero(log_message, sizeof(log_message));
      sprintf(log_message, "## LIGHT ## Recurring acquisition. Ambient Light Lux: %2.4f.", ambient_lux);
      LOG(mq_logger, log_message);
		...
		if(light_asynch == true)
		{
			...
			if(mq_receive(mq_light, (char *)&request, sizeof(mq_temp_light_payload_t), 0) < 0)
				{
          bzero(log_message, sizeof(log_message));
          sprintf(log_message, "## LIGHT ## Error receiving aynchronous request.");
          LOG(mq_logger, log_message);
        }

      bzero(log_message, sizeof(log_message));
      sprintf(log_message, "## LIGHT ## Asynchronous request received.");
      LOG(mq_logger, log_message);
...
void light_service_request(mq_temp_light_payload_t *request, char *response, int i2c_bus_desc)
{
    ...
    if(retval < 0)
    {
        sprintf(response, "Error accessing I2C slave device: %s", strerror(errno));
        pthread_exit(&retval);
    }
    ...
    {
      bzero(log_message, sizeof(log_message));
      sprintf(log_message, "## LIGHT ## Unable to send response to Sock_Comm_Task.");
      LOG(mq_logger, log_message);
    }
